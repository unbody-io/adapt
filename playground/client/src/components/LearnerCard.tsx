...
export function LearnerCard({ learner, anchor, onClose, onAsk }: Props) {
			...
			>
				...
				<div style={s.header}>
					<div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
						<span style={s.name}>{learner.name}</span>
						<span style={s.typeBadge}>{learner.type}</span>
					</div>
				...
				<div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
					<span style={activityStyle(learner.activity)}>
						{learner.activity === "idle" ? "idle" : learner.activity}
					</span>
				...
				<div style={s.section}>
					<div style={s.row}>
						...
						<span style={s.metricValue}>
							{learner.understandingSize > 0
								? learner.type === "list"
									? `${learner.understandingSize} items`
									: formatSize(learner.understandingSize)
								: "—"}
						</span>
					</div>
				</div>
				...
				{/* Description */}
				{learner.description && (
					<p style={s.description}>{learner.description}</p>
				)}
