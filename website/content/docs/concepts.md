...
Adapt ships two neuron types: **TextNeuron** and **ListNeuron**. When Brain auto-decomposes, the LLM chooses between them per domain — text for pattern synthesis, list for tracking distinct items. When in doubt, it defaults to text.
...
`inject()` feeds data to neurons for learning — it flows through the observe → understand pipeline. `signal()` feeds metadata to the evolution system — it buffers in the evaluator and triggers structural decisions (create, merge, split, update, delete neurons). Data goes through `inject()`. Steering goes through `signal()`.
